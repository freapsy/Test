...
// Erstelle die initiale Whitelist
exports.createInitialWhitelist = functions.https.onRequest(async (req, res) => {
  try {
    // Überprüfe, ob die Whitelist bereits existiert
    const whitelistRef = db.collection('whitelist');
    const snapshot = await whitelistRef.get();
    
    if (!snapshot.empty) {
      return res.status(400).json({ error: 'Whitelist existiert bereits' });
    }
    
    // Erstelle den ersten Whitelist-Eintrag
    await whitelistRef.add({
    ...
    return res.json({ success: true, message: 'Whitelist erfolgreich erstellt' });
  } catch (error) {
...
// Füge eine neue E-Mail zur Whitelist hinzu
exports.addToWhitelist = functions.https.onRequest(async (req, res) => {
  try {
    ...
    // Überprüfe, ob die E-Mail bereits in der Whitelist ist
    const whitelistRef = db.collection('whitelist');
    const snapshot = await whitelistRef.where('email', '==', email.toLowerCase()).get();
    
    if (!snapshot.empty) {
      return res.status(400).json({ error: 'E-Mail existiert bereits in der Whitelist' });
    }
    ...
    return res.json({ success: true, message: 'E-Mail zur Whitelist hinzugefügt' });
  } catch (error) {
...
// Aktualisiere einen Whitelist-Eintrag
exports.updateWhitelistEntry = functions.https.onRequest(async (req, res) => {
  try {
    const { id, isActive, role, notes } = req.body;
    
    if (!id) {
      return res.status(400).json({ error: 'ID ist erforderlich' });
    }
    
    const docRef = db.collection('whitelist').doc(id);
    ...
    return res.json({ success: true, message: 'Whitelist-Eintrag aktualisiert' });
  } catch (error) {
    console.error('Fehler beim Aktualisieren des Whitelist-Eintrags:', error);
    return res.status(500).json({ error: 'Interner Server-Fehler' });
  }
});
